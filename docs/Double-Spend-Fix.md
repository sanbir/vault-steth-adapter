...
### R2 — Guard retains `DEFAULT_ADMIN_ROLE`; borrower owns the guard — Medium (design fragility, not exploitable on current bytecode)
The PledgeGuard holds `DEFAULT_ADMIN_ROLE` on the Dashboard and the borrower owns the guard. The fix relies on the guard exposing **no** `grantRole`/`execute`/ownership-transfer forwarder, so on the current immutable bytecode the borrower cannot self-grant `WITHDRAW_ROLE`. But the defense is "no forwarding function," not "no capability" — there is no defense-in-depth. **Recommendation (open decision):** either have the guard renounce `DEFAULT_ADMIN_ROLE` after wiring (freezes the role graph — most secure, least flexible), or set the role-admin of `WITHDRAW_ROLE` to a neutral party the borrower does not control. This is a Factory-level change with a product tradeoff (vault recoverability after full unwind) and is left for an explicit decision.
